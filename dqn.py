...
class DQNAgent:
    ...
    def predict(self, state, target_model=False):
        X = np.reshape(state, [len(state), self.state_space[0]])
        if target_model:
...
class Env:
    ...
    def run(self, n_times = 100):
        ...
        print("Average action count : {}".format(sum(action_counts)/n_times))
...
if __name__ == '__main__':
    ...
    # def save_episode_image(self):
    #     save_episode = list(range(len(self.total_episode_count) // 10)
    #     pass
